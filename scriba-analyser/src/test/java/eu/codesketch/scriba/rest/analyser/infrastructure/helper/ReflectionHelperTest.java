...
import static eu.codesketch.scriba.rest.analyser.infrastructure.helper.ReflectionHelper.getAnnotatedMethods;
import static eu.codesketch.scriba.rest.analyser.infrastructure.helper.ReflectionHelper.getClassChain;
import static eu.codesketch.scriba.rest.analyser.infrastructure.helper.ReflectionHelper.getDecorators;
...
public class ReflectionHelperTest {
    ...
    public void testGetAnnotations_annotatedBaseClass() {
        // act
        List<eu.codesketch.scriba.rest.analyser.domain.model.decorator.Descriptor> annotations = getDecorators(AnnotatedBaseClass.class);
        // assert
        assertEquals(1, annotations.size());
        eu.codesketch.scriba.rest.analyser.domain.model.decorator.Descriptor annotation = annotations.iterator().next();
    ...
    public void testGetAnnotations_annotatedClass() {
        // act
        List<eu.codesketch.scriba.rest.analyser.domain.model.decorator.Descriptor> annotations = getDecorators(AnnotatedClass.class);
        // assert
        assertEquals(2, annotations.size());
        int level = annotations.size() - 1;
        for (eu.codesketch.scriba.rest.analyser.domain.model.decorator.Descriptor annotation : annotations) {
            assertEquals(Path.class, annotation.annotationType());
