...
import static eu.codesketch.rest.scriba.analyser.helper.ReflectionHelper.getClassChain;
...
public class ReflectionHelperTest {
    ...
    public void testGetAnnotations_annotatedBaseClass() {
        // act
        List<eu.codesketch.rest.scriba.analyser.model.Decorator> annotations = getDecorators(AnnotatedBaseClass.class);
        // assert
        assertEquals(1, annotations.size());
        eu.codesketch.rest.scriba.analyser.model.Decorator annotation = annotations.iterator().next();
    ...
    public void testGetAnnotations_annotatedClass() {
        // act
        List<eu.codesketch.rest.scriba.analyser.model.Decorator> annotations = getDecorators(AnnotatedClass.class);
        // assert
        assertEquals(2, annotations.size());
        int level = annotations.size() - 1;
        for (eu.codesketch.rest.scriba.analyser.model.Decorator annotation : annotations) {
            assertEquals(Path.class, annotation.annotationType());
