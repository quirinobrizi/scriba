...
import codesketch.scriba.annotations.ApiProduces;
 ...
 */
@Singleton
public class ApiProducesAnnotationIntrospector implements Introspector {
    ...
    public void instrospect(DocumentBuilder documentBuilder, Descriptor descriptor) {
        ApiProduces apiProduces = descriptor.getWrappedAnnotationAs(ApiProduces.class);
        documentBuilder.setOrReplaceProducibleTypes(apiProduces.value(), descriptor.isOnMethod());
    }
     ...
     */
    @Override
    public Class<ApiProduces> type() {
        return ApiProduces.class;
    }
