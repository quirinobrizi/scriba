...
import codesketch.scriba.annotations.ApiConsumes;
 ...
 */
@Singleton
public class ApiConsumesAnnotationIntrospector implements Introspector {
    ...
    public void instrospect(DocumentBuilder documentBuilder, Descriptor descriptor) {
        ApiConsumes apiConsumes = descriptor.getWrappedAnnotationAs(ApiConsumes.class);
        documentBuilder.setOrReplaceConsumableTypes(apiConsumes.value(), descriptor.isOnMethod());
    }
     ...
     */
    @Override
    public Class<ApiConsumes> type() {
        return ApiConsumes.class;
    }
